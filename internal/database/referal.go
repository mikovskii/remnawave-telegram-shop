...
func (r *ReferralRepository) Create(ctx context.Context, referrerID, refereeID int64) (*Referral, error) {
	query := sq.Insert("referral").
		...
		Suffix("RETURNING id, referrer_id, referee_id, used_at, bonus_granted, paid_at, earned_days").
	...
	var ref Referral
	if err := row.Scan(&ref.ID, &ref.ReferrerID, &ref.RefereeID, &ref.UsedAt, &ref.BonusGranted, &ref.PaidAt, &ref.EarnedDays); err != nil {
...
func (r *ReferralRepository) FindByReferrer(ctx context.Context, referrerID int64) ([]Referral, error) {
	query := sq.Select("id", "referrer_id", "referee_id", "used_at", "bonus_granted", "paid_at", "earned_days").
		From("referral").
	...
	for rows.Next() {
		var ref Referral
		if err := rows.Scan(&ref.ID, &ref.ReferrerID, &ref.RefereeID, &ref.UsedAt, &ref.BonusGranted, &ref.PaidAt, &ref.EarnedDays); err != nil {
...
func (r *ReferralRepository) StatsByReferrer(ctx context.Context, referrerID int64) (*ReferralStats, error) {
	query := sq.Select(
		"COUNT(*)::INTEGER",
		"COUNT(paid_at)::INTEGER",
		"COALESCE(SUM(earned_days), 0)::INTEGER",
	).
		From("referral").
		Where(sq.Eq{"referrer_id": referrerID}).
		PlaceholderFormat(sq.Dollar)
	...
	sql, args, err := query.ToSql()
	if err != nil {
		return nil, fmt.Errorf("failed to build referral stats query: %w", err)
	}

	var stats ReferralStats
	if err := r.pool.QueryRow(ctx, sql, args...).Scan(&stats.TotalReferrals, &stats.PaidReferrals, &stats.EarnedDays); err != nil {
...
func (r *ReferralRepository) FindByReferee(ctx context.Context, refereeID int64) (*Referral, error) {
	...
	var ref Referral
	err = r.pool.QueryRow(ctx, sql, args...).Scan(&ref.ID, &ref.ReferrerID, &ref.RefereeID, &ref.UsedAt, &ref.BonusGranted, &ref.PaidAt, &ref.EarnedDays)
	if err != nil {
